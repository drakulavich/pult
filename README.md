...
With the flag, every render reads the file and prints `load 36` in the colour of zapara's level for that number: calm green, warming yellow, heating red, fried bold red. Behind it come the two numbers that say whether it is time to rest: `streak 2h46`, how long your own prompts have kept coming without a pause of ten minutes, in any session, and `day 9h15`, the day's active time, counted in five-minute slots covered by your prompts and by the gaps between prompts of one streak. Agents working while you are away extend neither; that is zapara 0.3.1's presence rule. Both are grey while they are ordinary and turn yellow and then red once they are not (the streak at one and two hours, the day at six and eight), so the line does not shout in green all day; each is left out while it is zero. An hour with no activity yet prints `load -`. When the file's `asOf` is older than five minutes, or its `date` is not today's local date, or the file is missing or fails validation, pult starts `zapara status` in the background (the `zapara` on your `PATH`; the wrapper adds Bun's global bin directory, `~/.bun/bin` unless `BUN_INSTALL` or `BUN_INSTALL_BIN` says otherwise, which is where `bun install -g @drakulavich/zapara` puts it), does not wait for it, and prints the last value it has, dimmed, or nothing. The next render, thirty seconds later, reads the fresh file. Without a `zapara` installed nothing is started and the segment shows what the file has, if anything. A broken zapara costs one background start per render and an empty segment, never an error in the line; two starts at once are harmless, because zapara writes its file atomically.

The file is validated field by field before anything is printed, and a file that does not pass is treated as no data. The streak and the day are accepted up to 1500 minutes rather than 1440, because the day the clocks go back has 25 hours. A file about another day is stale however recently it was written, which is what a file from 23:59 read at 00:01 is. The file format and this refresh contract are documented in zapara's spec, `docs/superpowers/specs/2026-09-19-zapara-status-file-design.md`.
